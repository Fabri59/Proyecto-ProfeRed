const STORAGE_KEY = "peti_saas_simple_v2";
...
function migrate(data) {
  if (data.organizations && data.users) {
    data.organizations.forEach((org) => {
      org.peti.assignedResponsibles ||= {};
      org.peti.data.info.ruc ||= "";
    });
...
function registerCompany(event) {
  ...
  if (db.organizations.some((org) => org.peti.data.info.ruc === $("#regRuc").value.trim())) {
    toast("Ese RUC ya está registrado.");
    return;
  }
  const org = {
    id: uid("org"),
    name: $("#regCompany").value.trim(),
    peti: createPeti($("#regCompany").value.trim(), "", $("#regRuc").value.trim()),
  };
  org.peti.data.info.manager = $("#regName").value.trim();
...
function openCreateUserModal() {
  ...
  `, [
    { text: "Cancelar", className: "ghost-btn", onClick: closeModal },
    { text: "Crear usuario", className: "primary-btn", onClick: () => {
      const values = {};
      $("#modalForm").querySelectorAll("[data-field]").forEach((input) => { values[input.dataset.field] = input.value.trim(); });
      if (!values.name || !values.email || !values.password || !values.role) return toast("Complete todos los campos obligatorios.");
      if (db.users.some((u) => u.email.toLowerCase() === values.email.toLowerCase())) return toast("Ese correo ya existe.");
      db.users.push({ id: uid("usr"), organizationId: currentOrg().id, name: values.name, email: values.email.toLowerCase(), password: values.password, role: values.role, area: values.area, modulePermissions: {} });
      persist();
      closeModal();
